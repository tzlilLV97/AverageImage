...
from .utils import prepare_image, q_sample, process_images, prepare_mask, custom_PCA, custom_PCA_2
...
class Kandinsky2_1:
    ...
    def generate_img(
        ...
        if self.use_image_enc:
            ...
            print("samples shape before encoder is ", samples.shape)
        ...
        for i in range(len(images_texts)):
            if image_emb is None:
                if type(images_texts[i]) == str:
                    embedding = self.generate_clip_emb(
                        images_texts[i],
                        batch_size=1,
                        prior_cf_scale=prior_cf_scale,
                        prior_steps=prior_steps,
                    ...
                    image_emb = weights[i] * embedding
                else:
                    image_emb = self.encode_images(images_texts[i], is_pil=True)
                    image_embeddings.append(image_emb)
                    image_emb = weights[i] * image_emb
                    #image_emb *= weights[i]
            else:
                if type(images_texts[i]) == str:
                    embedding = self.generate_clip_emb(
                    ...
                    image_emb = image_emb + weights[i] * embedding
                    image_embeddings.append(embedding)
                else:
                    embedding = self.encode_images(images_texts[i], is_pil=True)
                    image_emb += embedding * weights[i]
                    image_embeddings.append(embedding)
        image_embeddings = torch.cat(image_embeddings, dim=0)
        old_image_emb = image_emb
    #    sanity_chk = torch.sum(image_emb * torch.tensor(weights).unsqueeze(1).to(self.device, dtype=self.model_dtype), dim=0)
        if PCA_components > 0:
            print("before calling PCA, which to amplify is ", which_to_amplify)
            image_emb = custom_PCA(image_embeddings, PCA_components, device=self.device, model_dtype=self.model_dtype, which_to_keep=which_to_keep, weights=weights,\
                                   which_to_reconstruct=which_to_reconstruct, amplify=amplify, which_to_amplify=which_to_amplify)

            # print("MSE : ", torch.nn.functional.mse_loss(sanity_chk, image_emb))
    ...
    def gen_avg_amp(
        ...
        print(
            f"Generate Average Image after amplification of specific components, PCA components : {PCA_components}, which to keep : {which_to_keep}, and the image that will be amplified : {which_to_amp}, seed : {seed}"
        )
        assert len(images_texts) == len(weights) and len(images_texts) > 0
        image_embeddings = []
        # generate clip embeddings
        ...
        #    sanity_chk = torch.sum(image_emb * torch.tensor(weights).unsqueeze(1).to(self.device, dtype=self.model_dtype), dim=0)
        if PCA_components > 0:
            image_emb = custom_PCA_2(image_embeddings, PCA_components, device=self.device, model_dtype=self.model_dtype,
                                   which_to_keep=which_to_keep, weights=weights,
                                   which_to_amplify=which_to_amp, amplify=amplify)
            # print("MSE : ", torch.nn.functional.mse_loss(sanity_chk, image_emb))
        image_emb = image_emb.repeat(batch_size, 1)
        if negative_decoder_prompt == "":
            zero_image_emb = self.create_zero_img_emb(batch_size=batch_size)
    ...
    #    sanity_chk = torch.sum(image_emb * torch.tensor(weights).unsqueeze(1).to(self.device, dtype=self.model_dtype), dim=0)
        if PCA_components > 0:
            image_emb =  amp * custom_PCA(image_embeddings, PCA_components, device=self.device, model_dtype=self.model_dtype, which_to_keep=which_to_keep, weights=weights, which_to_reconstruct=which_to_reconstruct)
            # print("MSE : ", torch.nn.functional.mse_loss(sanity_chk, image_emb))
        image_emb = image_emb.repeat(batch_size, 1)
        if negative_decoder_prompt == "":
            zero_image_emb = self.create_zero_img_emb(batch_size=batch_size)
